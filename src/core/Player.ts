...
export default class Player {
  ...
  decipher(
    ...
    if (this.sig_sc && (signature_cipher || cipher)) {
      ...
      Log.info(
        TAG,
        `Transformed signature from ${args.get("s")} to ${signature}.`
      );
    ...
    switch (client) {
      case "WEB":
        ...
        break;
      case "WEB_REMIX":
        url_components.searchParams.set(
          "cver",
          Constants.CLIENTS.YTMUSIC.VERSION
        );
        break;
      case "WEB_KIDS":
        url_components.searchParams.set(
          "cver",
          Constants.CLIENTS.WEB_KIDS.VERSION
        );
        break;
      case "ANDROID":
        url_components.searchParams.set(
          "cver",
          Constants.CLIENTS.ANDROID.VERSION
        );
        break;
      case "ANDROID_MUSIC":
        url_components.searchParams.set(
          "cver",
          Constants.CLIENTS.YTMUSIC_ANDROID.VERSION
        );
        break;
      case "TVHTML5_SIMPLY_EMBEDDED_PLAYER":
        url_components.searchParams.set(
          "cver",
          Constants.CLIENTS.TV_EMBEDDED.VERSION
        );
        break;
    }
  ...
  static async fromCache(
    ...
    if (version !== Player.LIBRARY_VERSION) return null;
  ...
  async cache(cache?: ICache): Promise<void> {
    if (!cache || !this.sig_sc || !this.nsig_sc) return;
